<template>
  <div class="product-list">
    ...
    <div class="page-part">
      <mu-list>
        <mu-list-item v-for='(item,index) in productList' :key='item.productKey' :title='item.productName' :describeText="item.productDesc" :open='false' class='group' toggleNested>
          ...
          <mu-list-item v-for='sub in item.deviceList' :key='sub.deviceId' :title='sub.deviceAlias'
                        :describeText="sub.deviceName" slot='nested'>
            <mu-float-button v-if='!sub.groupName' slot='right' icon="add" mini class="add-group"
                             @click='openAllocGroup(sub.deviceId)'/>
          </mu-list-item>
        </mu-list-item>
      </mu-list>
    </div>
